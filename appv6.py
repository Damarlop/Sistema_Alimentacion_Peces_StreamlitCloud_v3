...
if menu == "Ver especies disponibles":
    ...
    st.header("📜 Historial de Alimentaciones (Filtrado por especie)")
    historial = sistema.leer_historial_csv()
    if historial:
        especies_disponibles = sorted(set(entry["especie"] for entry in historial))
        especie_filtrada = st.selectbox("Filtrar por especie:", ["Todas"] + especies_disponibles)
        if especie_filtrada != "Todas":
            historial = [entry for entry in historial if entry["especie"] == especie_filtrada]
        st.dataframe(historial)
    else:
